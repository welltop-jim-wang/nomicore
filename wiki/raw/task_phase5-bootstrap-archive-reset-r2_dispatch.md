...
| 16 | 07:27 | SA3 | Phase 3 修复轮（R-FIX-1 + F-1） | (pending) | SA4 必须修复项回流：resetReplica 入口 expected 快照校验（复用 snapshotReplicationIdentityRef + 既有 INVALID_IDENTITY_ISSUE，零新冻结码）+ 敌意 16 形态测试锚（probeCalls 空分界断言）；顺带 F-1 observer 事件断言 |
