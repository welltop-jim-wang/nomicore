...
| `docs/adr/0014-vfsl-validated-jsonl-and-framed-sidecar-change-log.md`（ALLOW） | 见 §4 |
...
| `packages/namespace-diagnostic-log/AGENTS.md`（ALLOW） | Boundaries 新增：sync File adapter emit 不得在 namespace write slot 内接线（ADR-0014 amendment MUST；接线归 #149–#151/#155）；sequence 提交点纪律（definitive 复用 / ambiguous 封闭，禁回退「分配即消耗」） |
...
## 4. ADR-0014 amendment 落点

`docs/adr/0014-vfsl-validated-jsonl-and-framed-sidecar-change-log.md`（状态保留 `accepted`，ADR-0011 正文未动）：
...
| lineage：emit 在 write slot 内的接线风险 | 已在 ADR-0014 amendment 以 MUST 成文；实施归 #149–#151/#155（DENY LIST 未动） |
