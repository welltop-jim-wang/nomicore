...
- **任务类型**: 功能开发（Feature）
- **修订轮次**: R3（2026-08-20，SA2 复审 r2 reject 后修订）。R1 基线：commit `3e9a045`（2026-08-19 首版）；R2：commit `38be0fc`（SA2 首轮 9 项发现处置）。R3 输入：SA2 复审 `wiki/raw/task_vfsl-validate-snapshot_sa2_review_r2.md`（verdict: reject——R1 两 CRITICAL 已确认修复；R2-1/R2-2 HIGH + R2-3/R2-4 MEDIUM + R2-5/R2-6 LOW，全部为论证/标定层修正、不动架构主线）——逐项处置见 §16 R3 回应表；R3 实质改动集中在 §6.4（定理分列 + 预算二次项 + lookMemo）、§5.7-3（组合包络乘法推导 + WORK_LIMIT 重标定）、§3.4/§4/§4.1（计费补行 + preview 有界化 + 字段名索引）、§6.2/§6.2.1（转义统一 + `\|` 行 + 混合引擎否决）
...
| **preview 有界序列化（R3 补）**：按产出常数上界计费 | 每次调用 48 |

R3 补行的动机（SA2 R2-3 攻击构造）：封闭对象收到 10⁸ 个未知键的快照——未知键不触发 validateValue，R2 计费表下循环体只有免费的 emit/overflow++，`ctx.work` 几乎不增长，预算对该遍历面**失明**；补行后 10⁸ 键 → 10⁸ 单位 > WORK_LIMIT → loud `WorkBudgetExceeded`（10⁶ 键 ≈ 10⁶ 单位，照常完成并返回 101 条——SA2 IT 方向「有限时间内返回、不触发预算」兑现）。配套实现约束见 §4.1（字段名索引 Map，消除键数 × 字段数的未计费乘积）与 §4（preview 有界序列化，禁止全量 stringify）。
...
`preview(value)` = **有界序列化（R3 定稿）**：按 JSON 风格增量序列化，**产出满 40 字符即提前终止**（终止则追加 `…`）——**禁止**实现为 `JSON.stringify(value).slice(0, 40)`（全量 stringify 对深/大值是未计费的 O(值规模) 序列化，≤100 次 emit 可放大百倍，SA2 R2-3）；提前终止使成本被产出字符数封顶（深嵌套在 40 字符处停、长键/长数字在 40 字符处停——探索深度 ≤ 40），每次调用计 48 单位（§3.4 计费行）。诊断可读性用途不变（枚举消息专用）。
...
2. **非共享深乘积类被构造性兜底**：嵌套联合 × 多成员 × 多值位置可造出 distinct 对数量本身巨大的合法 schema（无共享子图可命中）——多项式但乘积可巨大。全局工作预算按**执行计费**（§3.4 计费点全覆盖 validateValue/countIssues/contradicts/模拟步/lookMemo 命中/编译/**键与元素访问**/**emit 与 overflow 记账**/**preview**——R3 补行后纯遍历面不再失明），2×10⁸ 上限硬封顶任意调用的总工作量，超限 loud fail-closed。对照 §6.1 否决「静态风险分析」的判据（「漏报即挂死，防护承诺破产」）：计费式预算**无识别面即无漏报面**——构造性成立（R2-3 三个盲区堵死后）。
3. **合法调用的组合包络（R3 重写为显式乘法推导 + v1 快照规模契约冻结；R2 的单点标定作废）**：
   ...
   **乘法链（自上而下，每个乘数显式）**：
   ...
   - + 节点访问乘数 = 每节点全程 ≤ ~10 单位（validateValue 进入 1 + 键/元素访问 1 + emit 1 + preview 48 仅枚举位）。

   **v1 快照规模契约（冻结，「支持 ≤ X 次 Pattern 应用 / ≤ Y 规模快照」）**：
...
match(regex, input): boolean          # test 语义：非锚定搜索（存在任一起点的前缀匹配即 true）
  ...
  lookMemo: Map<Look 指令, boolean[len+1]>   # (Look, pos) 结果记忆化（R3 新增，调用局部）：
...
- **活内存**：当前/下一状态集 + 闭包暂存 ≤ 3 × \|prog\| 槽，外加编译期类集合（\|prog\| 级）与 lookMemo（R3：≤ L×(len+1) 布尔槽，且每槽写入前必已付出 ≥ 1 计费步的求值 → 总槽位被步数预算封顶，4M 钳制下 ≤ 4M 槽 ≈ MB 级）——无回溯栈、无捕获副本、无预算外的随步数增长分配（#6 的内存上界即此：R1 的 `(pc,pos,captures 副本)` 栈已整体消除，OOM 绕过 E100 的路径不复存在）。
...
- **T2 类**：`L × m_sub × f_sub × (len+2)² > 512 × len² + 1_024 × len + 16_384`，或已触 4M 钳制——前瞻负载（L × m_sub × f_sub 乘积）超二次包络。
...
| R2-3 | MEDIUM：计费表三个盲区（未知键扫描 / emit·overflow / preview 全量 stringify）——键与元素访问计费 + preview 有界化 | ✅ | §3.4（计费表补 4 行 + 攻击构造对账）、§4（preview 重定义）、§4.1（字段名索引） | 补行：键被访问（含纯遍历未知键）每键 1 / 数组元素被访问（含计数态）每元素 1 / emit·overflow 每次 1 / preview 每次常数 48；10⁸ 未知键构造 → 10⁸ 单位 > 2×10⁸ 上限的 loud 路径成文（10⁶ 键照常 101 条返回）；preview 定稿**增量序列化 40 字符提前终止**，禁止 `JSON.stringify().slice()`（探索深度 ≤ 40、成本被产出封顶）；§4.1 字段名索引 Map（调用局部）消除键数 × 字段数未计费线性扫描 |
...
8. **预算公式与记忆化逐字实现（R3，防 R2-1 类走样）**：单匹配 `budget = min(4_000_000, max(8_192, 1_024×len + 512×len² + 16_384))`——二次项**不得省略**（省略即 SA2 攻击构造 `(?=.*;)z` × 202 字符在实现面重现「合法值被误拒」）；`lookMemo` (Look, pos) 记忆化**不得省略**（省略则嵌套前瞻回到 len^(d+1)）；全局 `WORK_LIMIT = 200_000_000`（§3.4/§5.7-3 重标定后数值——实现 16M 会使 3_000 键旗舰场景误触顶）。
