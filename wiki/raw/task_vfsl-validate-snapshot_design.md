...
- **任务类型**: 功能开发（Feature）
- **修订轮次**: R5（2026-08-20，SA2 四审 reject〔轻度〕后修订）。R1 基线：commit `3e9a045`（2026-08-19 首版）；R2：commit `38be0fc`（SA2 首轮 9 项发现处置）；R3：commit `3c9b528`（复审 r2 六项发现处置——SA2 三审确认全部实质落实）；R4：commit `f61748d`（三审 r3 四项发现处置——SA2 四审确认 R3 四项全部实质落实）。R5 输入：SA2 四审 `wiki/raw/task_vfsl-validate-snapshot_sa2_review_r4.md`（verdict: reject 轻度——R3 四项全部确认实质落实；剩 R4 新发现的契约表 T1/T2 行条件 m 缺口与 §6.3 槽位量纲标签，修法 < 10 行，预告修完若无新发现五审直接放行 SA3）——逐项处置见 §16-R5 回应表；R5 实质改动集中在 §5.7-3（契约表 T1/T2 行条件列补冻「单次应用定理界 ≤ 2.7×10⁴ / ≤ 10⁶」+ 合计行补正则编译计费括注 + R5 补冻说明段）与 §6.3/§5.7-4（lookMemo 槽位量纲修正：每槽 ≥ 2 计费步 → 槽 ≤ 2M、峰值 ~50–96MB）——不动任何冻结数值（WORK_LIMIT 与全部行值）、架构、定理、引擎、接缝与红灯对账
...
3. **合法调用的组合包络（R3 重写为显式乘法推导 + v1 快照规模契约冻结；R2 的单点标定作废）**：
   ...
   **乘法链（自上而下，每个乘数显式；R4 补联合重入乘数——R3 版缺席使契约在最坏同排满下被自家数字证伪〔SA2 R3-2：1.35×10^8 + 5×10^7 + 5.1×10^7 = 2.36×10^8 > 2×10^8〕）**：
   - 单次 Pattern 应用成本上界 = §6.4 定理界（按类分列）：T1 类（前瞻-free）64 码元 × m≈135 × f=3 ≈ **2.7×10^4**；T2 类（含前瞻、L·m_sub·f_sub ≈ 15）256 码元 ≈ **10^6**（m≈135 / 外层 m≈8 是**例示值**而非天然上界——输入长度不约束 m，R5 起定理界本身已冻入下行契约表的条件列，推导见「R5 行条件补冻」）；
   ...
   **v1 快照规模契约（冻结，「支持 ≤ X 次 Pattern 应用 / ≤ Y 规模快照」；R4 最坏口径重排——合计 = 各行上限之和〔SA2 R3-2 修订要求〕）**：
   ...
   |---|---|---|
   | 前瞻-free Pattern 应用（单次输入 ≤ 64 码元；**R5 补冻：单次应用定理界 ≤ 2.7×10⁴**——按 64 码元角点计，⟺ 编译产物 m ≤ 136 指令，66×m×3 ≤ 2.7×10⁴） | ≤ **5_000 次** | 5_000 × 2.7×10^4 = 1.35×10^8 |
   | 含前瞻 Pattern 应用（单次 ≤ 256 码元、L·m_sub·f_sub ≤ 15；**R5 补冻：单次应用定理界 ≤ 10⁶**——按 256 码元角点计，⟺ 外层 m·f ≤ 24，258×(24+257×15) ≈ 10⁶） | ≤ **50 次** | 50 × 10^6 = 5×10^7 |
   ...
   | **合计保守界（= 各行上限之和；R5 补编译计费括注）** | | **1.35×10^8 + 5×10^7 + 3×10^6 + 4.8×10^3 = 1.88×10^8；+ 正则编译 ≤ 5_050 distinct × 136 指令 ≈ 6.9×10⁵（regexCache 同模式只编译一次 → distinct 数 ≤ 应用总数 5_050；§3.4 计费表编译行此前无契约行承载，数值被余量吸收但未声明——R5 入合计）→ 1.887×10^8 ≤ WORK_LIMIT = 2×10^8（余量 ~5.7%）** |
   ...
   **R5 行条件补冻（SA2 R4-1——「条件即上界」直接冻法）**：R4 行常数 2.7×10^4 = 66×135×3 / 10^6 = 258×(24+257×15) 均内嵌编译产物规模 m 的例示值（135 / 外层 m≈8），而行条件只冻结输入长度、前瞻负载与次数——**输入长度不约束 m**：`[a-z]{1,64}` 单元 ≈ 135 指令、多连拼线性放大（八连 ≈ 1_016 ≪ 10_000 程序规模上限，均前瞻-free 合法子集内）。SA2 攻击构造（5_000 键 × 64 码元非匹配值 × m ≥ 149）：5_000×198×149 + 其余行 = 200,514,800 > 2×10^8——契约四行全合规仍触顶；§6.4 包络表自家放行的 T1 上限 m ≲ 340 × 2_184 次应用亦触顶（两表在最坏同排满下互斥）。修法取 SA2 推荐的最干净写法：**定理界本身入行条件**——T1 行 ⟺ m ≲ 135（66×135×3 = 26_730 ≤ 2.7×10^4）；T2 行在 len ≤ 256、L·m_sub·f_sub ≤ 15 之下 ⟺ 外层 m·f ≲ 24（258×(24+257×15) = 1,000,782 ≈ 10^6，偏差 0.08% 在余量内）。修后**各行上限在行条件下是真上限**（三审对 R3-2 的同一验收标准）。m 超限的模式（八连拼类）不违反任何运行时安全：单匹配照常多项式完成或单匹配预算 loud（§6.4 包络域），全局侧按**超契约调用**处置（预算内照常完成、耗尽 loud `校验工作预算耗尽`，fail-closed ~1s）——契约作为充分条件的推导闭合；WORK_LIMIT、行值（5_000 / 50 / 3×10^6 / 100）与 §6.4 包络表零改动。
...
4. **内存上界**：memo 容量 65_536 条封顶（§3.4）+ NFA 模拟活内存 O(\|prog\|) 与 lookMemo **稀疏物化的被写槽位**（§6.3，R4——稀疏规约使「每槽写入前必已付费 → 槽位被步数预算封顶」的论证在字面读法下成立，稠密预分配禁令入附录 8）+ emit 侧 issue 物化 ≤ 101 条（R4 门控后计数态零消息字符串）→ 调用级内存 ≈ O(派生物规模 + 快照遍历栈 + MB 级账本 + lookMemo 被写槽峰值 ~50–96MB〔§6.3 R5 量纲修正：每槽 ≥ 2 计费步 → 槽 ≤ 2M × ~24–48B〕)，无 GB 面（R1 的捕获副本回溯栈已随 §6.3 引擎重设计整体消除——SA2 #6 的 OOM 绕过路径不复存在；R3-1 的稠密分配回归通道由稀疏规约封死）。
...
- **活内存**：当前/下一状态集 + 闭包暂存 ≤ 3 × \|prog\| 槽，外加编译期类集合（\|prog\| 级）与 lookMemo（R3：键空间 ≤ L×(len+1) 布尔槽；**R4 稀疏物化**：`Map<Look, Map<number, boolean>>` 只有被写的槽才占内存——「每槽写入前必已付费的求值」的封顶论证由此**对实际占用的内存成立**：每槽写入前必已付出 **≥ 2 计费步**（外层闭包访问该 Look 指令 1 步 + 锚定子模拟 ≥ 1 步——空前瞻 `(?=)` 的子闭包也要访问 Match 1 步，R5 修正 R4 的「≥ 1 步」粗算），被写槽位被步数预算封顶，4M 钳制下槽 ≤ **2M**；Map 条目 ~24–48B/槽 → 峰值 ≤ ~**50–96MB**（典型 KB–MB 级；对抗构造 200×`(?=)` × 10^4 码元 ≈ 1.33M 槽 ≈ 53MB，loud BudgetExceeded 后随调用释放），**无 GB 面、余量 ≥ 10×**；稠密预分配禁令见上行规约与附录 8——无回溯栈、无捕获副本、无预算外的随步数增长分配、**无计费外的大块物化**（#6 的内存上界即此：R1 的 `(pc,pos,captures 副本)` 栈已整体消除，OOM 绕过 E100 的路径不复存在；R3-1 的稠密分配面是唯一回归通道，已由存储规约封死）。
...
### §16-R5. SA2 四审（r4）逐条回应（2 项发现 → 修订映射）

| # | 要求（SA2 四审 §1/§2） | 是否落实 | 修订位置 | 修订内容摘要 |
|---|---|---|---|---|
| R4-1 | MEDIUM：契约表 T1/T2 行保守界常数内嵌编译产物规模 m 而行条件不冻结 m（m ≥ 149 × 5_000 次 × 64 码元 = 200,514,800 > 2×10^8 破契约；与 §6.4 包络表 m ≲ 340 × 2_184 次互斥）；计费表「正则编译一次」无契约行承载、合计漏一项（≤ 5,050×135 ≈ 6.8×10^5 被余量吸收但未声明）——T1/T2 行条件补 m 从句或改冻单次定理界（推荐后者「条件即上界」最干净）+ 合计补编译括注；修后各行上限须在行条件下是真上限 | ✅（取**「条件即上界」直接冻法**） | §5.7-3（契约表 T1/T2 行条件列、合计行、表后「R5 行条件补冻」说明段、乘法链 bullet 例示挂钩） | T1 行条件列补冻「单次应用定理界 ≤ 2.7×10⁴ ⟺ m ≲ 135 指令」、T2 行补冻「单次应用定理界 ≤ 10⁶ ⟺ 外层 m·f ≲ 24」——条件即上界，行上限在行条件下为**真上限**（三审对 R3-2 的同一验收标准）；说明段给出等价推导（66×135×3 = 26_730；258×(24+257×15) = 1,000,782 ≈ 10^6 偏差 0.08%）与攻击数字（m ≥ 149 全合规触顶 / 包络表 m ≲ 340 两表互斥），m 超限模式按**超契约调用**语义闭合（单匹配多项式完成或预算 loud、全局 fail-closed ~1s）；合计行补编译计费括注「+ ≤ 5_050 distinct × 135 ≈ 6.8×10^5 → **1.887×10^8** ≤ 2×10^8（余量 ~5.7%）」——编译行计费此前无契约承载，现已入合计算式；WORK_LIMIT 与全部行值（5_000/50/3×10^6/100/48）零改动 |
| R4-2 | LOW：§6.3「4M 钳制下 ≤ 4M 槽 ≈ MB 级」量纲失准——每槽写入前实为 ≥ 2 计费步（外层闭包访问 1 + 子模拟 ≥ 1）→ 槽 ≤ 2M；Map 条目 ~24–48B → 最坏 48–96MB（「无 GB 面」结论成立、余量 ≥ 10×，但非「MB 级」）；一句话改写 | ✅ | §6.3（活内存 bullet 一句）、§5.7-4（峰值口径括注） | §6.3 改写为「每槽 ≥ 2 计费步（外层闭包访问 1 步 + 锚定子模拟 ≥ 1 步——空前瞻 `(?=)` 的子闭包也要访问 Match 1 步，R5 修正 R4『≥ 1 步』粗算）→ 槽 ≤ **2M**；~24–48B/槽 → 峰值 ≤ ~**50–96MB**（典型 KB–MB 级；对抗构造 200×`(?=)` × 10^4 码元 ≈ 1.33M 槽 ≈ 53MB），**无 GB 面、余量 ≥ 10×**」；§5.7-4「无 GB 面」句同步挂 lookMemo 峰值口径（两节量纲一致，SA4/SA7 设计内存断言不再误设「MB 级」阈值）；R3-1 修订主体（稀疏规约/稠密禁令/附录 8 锚点）零改动 |

**修订无效模式自检（R5）**：R4-1 落在契约表行条件列与合计行算式（可检行为：SA4 以契约行为充分条件验证时，八连拼 m≈1_016 模式从「四行全合规仍触顶」变为「不满足 T1 行条件——超契约调用，loud 兜底语义在案」；合计从漏一项的 1.88×10^8 变为含编译声明的 1.887×10^8）；R4-2 落在 §6.3 槽位量纲句（可检行为：按修正后 50–96MB 上界设计的内存断言/监控阈值与对抗构造实测 ~53MB 同阶，不再差 1–2 个数量级）。两处均为实质改动，非旁注承认。
...
### R5 修订记录（2026-08-20，SA2 四审 r4 reject〔轻度〕后——两处局部补写，不动冻结数值/架构/定理/引擎/接缝/红灯对账）

- **§5.7-3（R4-1）**：契约表 T1/T2 行条件列补冻「单次应用定理界 ≤ 2.7×10⁴ / ≤ 10⁶」（⟺ m ≲ 135 / 外层 m·f ≲ 24——「条件即上界」冻法，行上限在行条件下为真上限）；表后新增「R5 行条件补冻」说明段（SA2 攻击数字 m ≥ 149 / 包络表 m ≲ 340 × 2_184 两表互斥、等价推导、m 超限的超契约调用语义闭合）；乘法链 bullet 例示值挂钩行条件；合计行补正则编译计费括注（≤ 5_050 distinct × 135 ≈ 6.8×10^5 → **1.887×10^8** ≤ 2×10^8，余量 ~5.7%）。
- **§6.3/§5.7-4（R4-2）**：lookMemo 槽位量纲修正——每槽 ≥ 2 计费步（外层闭包访问 1 + 锚定子模拟 ≥ 1）→ 槽 ≤ 2M、~24–48B/槽 → 峰值 ~50–96MB（典型 KB–MB 级），无 GB 面、余量 ≥ 10×；替换 R4 失准标签「≤ 4M 槽 ≈ MB 级」；§5.7-4 同步挂峰值口径。
- **§16/§17/头部（文档级）**：R5 回应表与修订记录、头部修订轮次链更新。
- **不变项**：接缝形状（§2）、三段联合算法（§5.2）、NFA 引擎架构/指令集/预算公式数值（§6.3 主体——仅量纲一句）、T1/T2 定理陈述与 §6.4 包络表（定理界本身未动，只是冻入行条件）、XML 检查器（§7）、截断语义（§8）、path 与遍历序（§9）、loud 边界（§10）、§11.1 测试缺陷处置、ALLOW/DENY LIST（§13）、协议假设与契约审计（§14/§15）、附录防走样指令——SA2 四审「仍屹立的项」（评审 §7）全部保留；WORK_LIMIT = 2×10^8 与全部行值零改动；R5 改动全部落在表条件列/合计括注/量纲句层（与四审结论「修法 < 10 行、两处均为行条件从句/量纲标签的局部补写」对齐）。
