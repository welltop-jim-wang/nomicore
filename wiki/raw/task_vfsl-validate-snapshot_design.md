...
3. **合法调用的组合包络（R3 重写为显式乘法推导 + v1 快照规模契约冻结；R2 的单点标定作废）**：
   ...
   **v1 快照规模契约（冻结，「支持 ≤ X 次 Pattern 应用 / ≤ Y 规模快照」；R4 最坏口径重排——合计 = 各行上限之和〔SA2 R3-2 修订要求〕）**：
   ...
   |---|---|---|
   | 前瞻-free Pattern 应用（单次输入 ≤ 64 码元；**R5 补冻：单次应用定理界 ≤ 2.7×10⁴** ⟺ 编译产物 m ≲ 135 指令） | ≤ **5_000 次** | 5_000 × 2.7×10^4 = 1.35×10^8 |
   | 含前瞻 Pattern 应用（单次 ≤ 256 码元、L·m_sub·f_sub ≤ 15；**R5 补冻：单次应用定理界 ≤ 10⁶** ⟺ 外层 m·f ≲ 24） | ≤ **50 次** | 50 × 10^6 = 5×10^7 |
   ...
   | **合计保守界（= 各行上限之和；R5 补编译计费括注）** | | **1.35×10^8 + 5×10^7 + 3×10^6 + 4.8×10^3 = 1.88×10^8；+ 正则编译 ≤ 5_050 distinct × 135 指令 ≈ 6.8×10⁵（regexCache 同模式只编译一次 → distinct 数 ≤ 应用总数 5_050；§3.4 计费表编译行此前无契约行承载，数值被余量吸收但未声明——R5 入合计）→ 1.887×10^8 ≤ WORK_LIMIT = 2×10^8（余量 ~5.7%）** |
