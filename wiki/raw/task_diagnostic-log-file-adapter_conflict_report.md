...
| 编号 | 标题 | 状态 | 相关 | 对照结论 |
|---|---|---|---|---|
| 0001 | VFSL 单一真相源 | accepted（含 2026-08-19 修订） | 中 | no-conflict。简报不新增 schema 文本（明示「不改 schema」）；内建冻结 record schema 的存在由 ADR-0014 明文决策（「writer 启动时编译一次内建 schema 并缓存」），且 0001 修订节已放行阶段态仓内 schema；「方言只增不改、未知 loud-fail」与简报 strict reader「不近似解释未知版本」同向。 |
...
| 0004 | vfsl-protocol 类型投影 | accepted | 弱 | no-conflict。本票不触及类型投影包；ADR-0014 被否方案「只用 TypeScript 类型、不做 VFSL 验证」与简报「records pass the built-in VFSL schema … validation」同向。 |
| 0005 | 投影生成管线 | accepted | 弱 | no-conflict。SchemaSource 接缝纪律针对仓内脚手架 schema 消费方；本票消费内建冻结 schema，非该场景。 |
| 0006 | DocPersistence 与 doc 三条目布局 | accepted（含三次修订） | 中 | no-conflict。简报不改 DocPersistence 契约；ADR-0014 明文「不修改 ADR 0006 的 snapshot Persistence」，日志布局（`namespaces/{namespaceId}/…`）与 0006 的 `{rootDir}/users/{userId}/{namespaceId}.snapshot` 是独立目录空间；temp+rename 原子替换（current.json）正是 0006 已验证模式。 |
...
| 0008 | NamespaceRuntime 读写能力与单序列器 | accepted（含稳定码注册修订） | 中 | no-conflict。简报「non-interference with producer results」与 ADR-0011/0012「日志不得引入第二个业务排序机构、emitter 不被 await、不延长 write slot」同向；ADR-0014「多 Runtime generation 共享同一 writer queue」属 adapter 内部实现，不碰 0008 sequencer。 |
| 0009 | NamespaceRegistry、租约与 Host 生命周期 | accepted | 弱-中 | no-conflict。ADR-0014「日志启用与配置是本地旁路状态、初始化失败不影响 namespace create」与 0009 lifecycle 正交；shutdown 不无限等待日志与 0009 shutdown 纪律同向。 |
...
| 0012 | VFSL 校验的 JSONL 与 framed sidecar 诊断日志格式 | accepted | 最高（主规范） | no-conflict。简报五条 AC 逐条映射 ADR-0014 条款（见下），无任何背离；范围切分（#153/#154/#155 排除）不违反 ADR——0012 是终态规范，未规定票边界，且简报自设「除非 ADR 0014 明确归属本票」的保守条款。 |
...
### 简报 AC ↔ ADR-0014 逐条映射（放行依据）

| 简报要求 | ADR-0014 对应条款 | 一致性 |
|---|---|---|
...
| 上下游事实：genesis 由 #152 adapter 内部构造、不改 schema | CONTEXT.md「genesis baseline record」术语已备案（设计 §10-J1）；ADR-0014「每个新 stream 尽力先记录…genesis baseline」 | 已备案，非冲突 |
...
1. **范围切分张力（非冲突）**：本票排除 #153（segment rolling / 打开与尾部恢复），但 ADR-0014 的「打开现有 stream 时 manifest fingerprint 匹配检查（不匹配 → 旧 stream 只读、新建 generation、不改旧 manifest）」与验收门槛 10 邻接本票 manifest/strict reader 范围。SA1 设计时须明确本票对「打开既有 stream」的支持边界，且任何切分不得违反 ADR-0014 已冻结条款。
2. **Genesis 构造**：adapter 内部构造 genesis baseline record、不改 schema——CONTEXT.md 已备案（设计 §10-J1），schema 已含 `recordKind: 'genesis-baseline'` 判别分支，本票不得扩词表、不得为 semantic emission 建第二份 VFSL（ADR-0014 明文禁令）。
