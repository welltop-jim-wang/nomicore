# 设计增补（R2.2.1）— issue #134 Round 2 修订轮（PR #146 评审 12 项 → R2-1..R2-12）

- **修订轮次**: **R2.2.1**（2026-08-28）——SA4 静态验尸窄门回流（verdict: reject，单一必修 **F-1**；报告 `task_namespace-lease-replication-session_round2_sa4_review.md`——机制本体/文档闭环/纪律面/敌意面全部通过，唯一破口 = §5.2 冻结规则表两行的登记锚未落位）。措辞层修订：① §5.2 两行收窄重构为三行——Date（可行锚：实际触发分支 = **跨形态分叉**——scratch round-trip 摊平 plain `{}` ⇒ 单侧白名单拒，非 proto 门路径）+ undefined/bigint（可行锚：忠实 round-trip、typeof fallthrough 拒）+ Map/Set/symbol/function（**种子面 loud throw 豁免登记**——`Y.Map.set` 即 throw「Unexpected content type」，比较层结构性不可达、无锚义务）；② §5.3 摘要行与 §15.2 登记行收口为与 §5.2 同一锚清单（含豁免）；③ 文末 R2.2 修订记录追加 F-1 小节（如实登记「冻结+锁定测试」承诺的两行此前缺锚、SA3 偏离登记亦静默缺位）。零代码触碰；SA3 随新文本补锚（§15.2 (ii)）。探针依据：SA4 独立实测 + SA1 本轮复测（Date/undefined/bigint 种子成功且可测；Map/Set/symbol/function 四型种子面均 loud throw）。详见文末「R2.2 修订记录 · F-1」。
- **R2.2**（2026-08-28）——SA3 交付（commit 8a68d82）后的三项就地裁决：① 认可偏离 1（§5.1 白名单物化前提被 yjs 13.6.32 实测反证——plain array/object 经 lib0 `writeAny` **原样存储**，非物化为 Y.Map/Y.Array；白名单修正为「Y.Map/Y.Array ∪ plain array ∪ plain object」；**SA2 R2.2 复审 M-1 指出 ADR L273 括注仍残留已作废前提原句——经 M-1 就地修正后设计/实现/ADR 三方逐字一致**）；② 认可发现 1（§19「订阅先于写⇒快照漂移不可观测」声称与 round-1 registry AC-2 ③ 事实不符——作废该声称 + 登记 AC-2 ③ 一行 fixture 时序演进授权）；③ 采纳发现 2 处置 (a)（spin fixture 测试末 close 收尾——§4.3(d)/§18 补测试隔离义务注记，SA7 满载协议保持）。另核：偏离 2（新文件 ~740 行）认可、偏离 3（SA2 #5 可选锚改由 SA7 可选承接）裁决登记。
...
| **Date**（非 plain 实例——可种子、可 round-trip）【R2.2.1 / SA4 F-1 收窄】 | live 侧保持 Date 实例；scratch 侧 round-trip 被 lib0 writeAny **摊平为 plain `{}`**（proto=Object） | 保守「已改变」——**实际触发分支 = 跨形态分叉**（scratch 侧 plain `{}` 在白名单、live 侧 Date 不在 ⇒ 单侧白名单即拒；非 plain 原型门路径——结果同为保守拒） | **SA3 新锚（R2.2.1 补——Date 经跨形态分叉拒，见 §15.2）** |
...
| **undefined / bigint**（契约外标量——可种子、**忠实 round-trip**：scratch 侧分别为 undefined / 10n）【R2.2.1 / SA4 F-1 收窄】 | 恒等（round-trip 不摊平） | 同型同值经 typeof fallthrough 保守拒（契约外——L31 值域外形态不得经 raw 判等放行） | **SA3 新锚（R2.2.1 补——见 §15.2）** |
| **Map / Set / symbol / function**（种子面 loud throw——比较层结构性不可达）【R2.2.1 / SA4 F-1 豁免登记】 | `Y.Map.set` 即同步 throw「Unexpected content type」（lib0 writeAny 域外——先于本判据的 Yjs 自身域门） | 种子面 loud 拒——**不达比较层**（豁免：无比较层锚可落） | **豁免登记（loud throw 即行为事实；SA3 可选种子面断言，无比较层锚义务）** |

- **键集先行**：容器级比较先比键集（存在性），再逐键值比较（round-1 `protectedMapEqual` 结构保留，仅替换值判等函数为 `protectedValueEqual`）——red #12（META 新键，值为对象 → 键集不等）保持拒绝零写入。
- **【R2.1 / SA2 #2 + R2.2 / 偏离 1】容器域两级裁决（与 §5.1 代码逐字一致）**：顶层（META/SCHEMA 键的直接值）走白名单门——plain array/object（yjs 原样存储的合法值形态）∪ Y.Map/Y.Array（显式构造容器）参与投影深比较，其余一切 AbstractType（Y.Text 等）与非 plain 实例保守拒（虽内容未变亦拒；**Date 的实际触发分支为跨形态分叉、Map/Set/symbol/function 在种子面即被 Yjs loud 拒不达比较层——见上表 R2.2.1 三行**）；嵌套（白名单容器内部）走投影摊平——契约外子值投影相等即放行（上表归一化边界行）。§5.1 代码的 `isWhitelistedValueContainer`/`projectOf` 即此两级裁决的实现，四处（代码/本表/§14 D-3/ADR L273）由 R2.1+R2.2 两轮修订对齐——前版「Y.Text 投影类型分叉 → 拒」表述矛盾（R2.1 消除）与「仅物化为 Y.Map/Y.Array」前提错误（R2.2 消除）均已修正。
...
### F-1 执行记录（SA4 静态验尸窄门回流——R2.2.1 措辞层修订，2026-08-28）

- **问题（如实登记）**：§5.2 规则表（自名「冻结 + 锁定测试」）的「Date / Map / Set 等非 plain 实例拒」与「undefined / bigint / symbol / function 保守拒」两行，锚列登记为「SA3 新锚（R2.2）」但 **SA3 从未交付这些锚**（22 用例清单零命中；偏离登记三项亦不含——静默缺位）：可行半边（Date / undefined / bigint——SA4 探针证实可种子、可测）缺测试锁，不可行半边（Map / Set / symbol / function——种子面即 loud throw）缺豁免登记。**「冻结 + 锁定测试」承诺在两行上落空**——本节补齐。
- **事实核验（双源）**：SA4 独立探针 + SA1 本轮复测（worktree `packages/namespace-runtime` 下 node 探针）一致：`meta.set('d', new Date(0))` 种子成功，live 保持 Date 实例、scratch round-trip 摊平为 plain `{}` ⇒ 比较层经**跨形态分叉分支**（单侧白名单）保守拒——**非 proto 门路径，结果同为保守拒**（前版「plain 原型门排除」的路径归因不实，已修正）；`undefined` / `10n` 种子成功且忠实 round-trip ⇒ typeof fallthrough 保守拒（末行纯粹形态）；`new Map()` / `new Set()` / `Symbol()` / 函数 种子面即 `Y.Map.set` 同步 throw「Unexpected content type」（lib0 writeAny 域外——**比较层结构性不可达**）。
- **修订落位（三处对齐 + 一处注记）**：① §5.2 两行重构为三行——Date 行（可行锚 + 跨形态分叉路径如实注明）、undefined/bigint 行（可行锚）、Map/Set/symbol/function 行（**豁免登记**：loud throw 即行为事实，SA3 可选种子面断言、无比较层锚义务）；② §5.3 摘要行补同一清单（①②③ 三段）；③ §15.2 登记行收口为同一三段清单（(i) 已交付 plain 半边 / (ii) R2.2.1 可行补锚——SA3 待补 / (iii) 豁免登记）；④ §5.2 两级裁决 bullet 增补行内指引（Date 分支 + 种子面不可达）。零代码触碰——锚落位由 SA3 按新文本执行（§15.2 (ii)）。
- **不改项**：白名单构成与比较语义零变化（三行重构只是锚登记与路径归因的如实化——拒绝结果在所有形态上与实现一致）；SA4 结论「机制本体与设计逐字一致」不受影响；SA2 N'-1 记录更正归 SA2（SA4 指定），不在本文件范围。
