...
## SA2 反馈逐条回应（R1 评审 verdict = pass；2 项勘误 + 3 项建议回流，2026-08-22）

评审报告：`wiki/raw/task_read-logical-value-at-path_rev1_sa2_review.md`（核心论证独立重推全部成立、66 例实测全绿、D16/D17/D18/D13 与 ADR/首轮契约零冲突）。本表为 SA2 攻击点 #1–#5 及 NOTE 项的逐条落实记录：

| 要求 | 是否落实 | 修订位置 | 修订内容摘要 |
|------|:--:|------|------|
| #1（MINOR，勘误）：§1.3「三项演化都会静默打开可达面」宣称过强——SA2 独立推演仅异构段消耗节点 kind 真正威胁归谬第 2 步，E309/yjs-undefined 两项归谬鲁棒（E309 放宽后标量成员走 M10 reject/终点 walkUnion 不经中段仲裁；yjs undefined 元素产 reject 非 missing）；应收敛主例、其余降级为事实前提表述 | ✅ | 摘要「为什么必须修」段 + §1.3 条 2 | 主例收敛为「**唯有**新增带异构段消耗的节点 kind（破坏第 2 步）」；E309/yjs-undefined 降级为「三源枚举与终点快照论证（INV-12）的**事实前提**，而非威胁向量」，并把 SA2 的 (a)/(b) 两条鲁棒性推演原文纳入 §1.3（含 read.ts:340-341 界内 undefined 下钻 → `carrierOf(undefined)` → reject 的行为引注）；「四个结构性事实」表述同步改为不含威胁向量定性的合取列举 |
| #2（MINOR，勘误）：SUP-2 护栏层数 22 → 26（supplementary.test.ts:96 `CHAIN_DEPTH = 26`） | ✅ | §3.4 成本段 | 「22 层」改「26 层」并附 `CHAIN_DEPTH = 26` 引注与勘误编号；实质论证（最坏路径不含 missing 短路受益）不变——与 SA2 复核结论一致 |
| #3（MINOR，建议）：value-first 新增成本面（中段 missing 后继续试探后序成员子树）无测试锚——SUP-2 全 required fixture 上不存在该成本面 | ✅ 采纳 | §3.4 护栏缺口注记 + §4 可选锚点 H-a + §8.1 可选文件条目 | §3.4 显式成文该缺口（渐近安全由 memo 摊销论证保证而非护栏锚定）；§4 新增 H-a 锚点规格（26 层链 × 中段 optional 缺席 `{ x?: L2; t1 } \| { x: L2; t2 }` 变体 + live 中段缺 x → `ok:true, value:undefined` 且 <2s，红灯触发 = memo 丢失/试探未摊销）；§8.1 可选测试文件条目同步更新 |
| #4（NOTE，建议）：mixed 反序锚（reject 先、missing 后）增量检测力说明——对「循环遇 reject 提前终止」提供第二锚；对「首 missing 即返回」（现行 bug 行为）两序均无检测力是观测等价必然非缺口 | ✅ 采纳 | §4 可选锚点 H-b（原 mixed 反序条目扩充） | H-b 规格按 SA2 红线思路重写（fixture/断言/红灯触发条件/两锚分工/R4-3 第一锚关系），标注「建议优先纳入」；「无检测力 = 观测等价必然」的定性原文纳入，防止未来误判为护栏缺口 |
| #5（NOTE，建议）：嵌套 union 语义一句成文 | ✅ 采纳 | §3.2（owner 四规则对照表之后） | 新增「嵌套 union 位」段：子 union 先按 D17 聚合自身三态、其结局作为成员结局参与外层记账（missing→`sawMissing`、value→外层首 value、reject→成员 reject）——递归自然传播、无双重仲裁 |
| NOTE（§6 实证基线附注）：grep 命中的 extract.ts / vfsl/index.ts 为 JSDoc/注释级提及非调用，可补一句澄清 | ✅ 采纳 | §6 caller 表「存量 caller」行 | grep 结果修正为 8 文件全列举，显式标注 extract.ts/vfsl-index 两处为 JSDoc/注释提及**非调用**（SA2 R1 实测澄清） |
| 附带自纠 | ✅ | §4 AC-R4 段 | 原文「落点文件见 §7 ALLOW 可选项」交叉引用笔误修正为 §8.1（文件清单位于 §8） |

**一致性自检（修订后复跑）**：「异构段消耗」在摘要/§1.3/回应表三处口径一致（唯一威胁向量，破坏第 2 步）；E309/yjs-undefined 定性统一为「事实前提非威胁向量」（摘要/§1.3/§3.4 缺口注记）；「26 层」出现于 §3.4/§4-H-a/§8.1 并与 supplementary.test.ts:96 实测一致（`grep -n "22 层"` 正文零命中，仅本表勘误引文内出现）；H-a/H-b 编号在 §3.4/§4/§8.1 三处互引一致；normative 条款（D16/D17/D18、三分法 M1-M10、INV-7 精确化/INV-12..14、§3.2 伪代码）**零改动**——本次回流全部为措辞收敛、事实勘误与可选锚点规格，SA2 R1 verdict = pass 的评审对象实质不变。
