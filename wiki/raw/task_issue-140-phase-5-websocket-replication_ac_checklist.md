...
| AC4 | degraded/retry/stale snapshot/diff recovery 在两种 adapter 上通过 | ✅ | 双 adapter degraded/retry：`packages/namespace-runtime/test/runtime-acceptance-degraded-two-adapter.test.ts`（Memory/File 平行验收）；Hub degraded 拒绝与恢复 diff：`packages/ws-replication/test/ws-replication-ac7-faults.test.ts`；断线写与重连 state-vector diff：`packages/ws-replication/test/ws-replication-ac6-resync-close.test.ts`；File stale snapshot / crash restart：`apps/yjs-server/test/phase5-three-instance-acceptance-red.test.ts` | 已建立逐项可追溯测试映射 |
...
| AC8 | typecheck、完整测试、aggregate no-emit、diff checks、Node matrix、Standards/Spec final review | ✅ | pre-merge 复验：`pnpm typecheck` exit 0；`pnpm test` 118 files / 1405 tests / Type Errors 0；`git diff --check` exit 0；PR CI Node 20/24 两矩阵均 SUCCESS；最终 Standards/Spec review 的 release blockers 已修复 | 本地与 CI 门禁通过 |

## 结论

所有 AC 均有可追溯的动态或静态证据；pre-merge 复验与 PR Node 20/24 CI 矩阵均通过，最终双轴 review 发现的 release blockers 已在推送前闭环。
