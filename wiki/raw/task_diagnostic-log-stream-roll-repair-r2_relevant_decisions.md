...
### ADR-0014 §打开与尾部恢复（accepted）——本轮主规范
...
### ADR-0014 §Writer、append 与背压（accepted，含 amendment）——链衔接与首引用豁免
...
### ADR-0014 §Stream 与 generation（accepted）——修复属 stream 内行为，不触发换代
...
### ADR-0014 §Segment rolling 与耗尽 + §VFSL record schema（accepted）——滚动种子与事件纪律背景
