...
> 冲突基准 = `docs/adr/` 全部 **13 个文件**（0001–0012，含两个撞号 0012：ADR-0014-LOG / ADR-0014-INSTANCE）+ 根 `CONTEXT.md`，逐个全读，无抽样。
...
### 3. open 路径建流无 genesis（vs ADR-0014-LOG「每个新 stream 尽力先记录 genesis baseline」）
...
### 4. Registry create/open/import 槽内的同步 File adapter fs（vs ADR-0014-LOG amendment write-slot 红线）
...
- **Host 数据删除联动顺延**（§8 非目标 4）：ADR-0014-LOG「Host 执行数据删除请求时必须同时调用日志删除能力」是条件条款，app 现无数据删除面 → 前件不成立；设计显式备案待相应票，非违反。
...
| #149/#151 emitSlot 槽外纪律 | `grep emitSlot packages/namespace-runtime/src/diagnostic.ts` | ✅ 文件头明文「emitSlot 由公共方法的 `.then` 回调调用」（ADR-0014 amendment C）；本票零改动该文件（DENY） |
