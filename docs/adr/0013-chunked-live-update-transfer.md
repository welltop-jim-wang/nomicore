...
- ~~SYNC_STEP2 diff 与 BOOTSTRAP_SNAPSHOT 分块（R3 揭示的另一半尾部）——同一 transfer 机制可平移，列为后续独立 capability（如 `CAP_CHUNKED_SYNC`），不在本 ADR 冻结~~ 【已由 ADR 0019 接替：`CAP_CHUNKED_SYNC` 协商 + 0x42 kind 形态，issue #295】；
