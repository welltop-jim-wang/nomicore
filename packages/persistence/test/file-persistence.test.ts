 ...
 *
 *   - `FilePersistence` class (options: rootDir + optional schedule/timer)
...
import { describeDocPersistenceContract } from '../src/testing.js'

const TEST_SCHEDULE: Partial<PersistenceSchedule> = { debounceMs: 10, maxDirtyMs: 50 }

const sleep = (ms: number): Promise<void> => new Promise((resolve) => setTimeout(resolve, ms))
/** Real-timer flush completion: debounce (10ms) fires well inside this window. */
const waitForFlush = (): Promise<void> => sleep(250)
...
describeDocPersistenceContract(async () => {
  ...
  const persistence = new FilePersistence({ rootDir: makeRootDir(), timer })
  return {
    persistence,
...
describe('FilePersistence', () => {
  ...
  it('fully restores SCHEMA/META/ROOT through a brand-new instance after save (crash restart)', async () => {
    ...
    const writerDoc = await seedAndFlush(writer, { userId: 'alice' }, 'ns-1', (doc) => {
      populateThreeEntryDoc(doc, 'ns-1')
    })
    await writer.dispose()

    const reader = new FilePersistence({ rootDir })
  ...
  it('isolates users: the same docId under different users lives in separate partitions', async () => {
    const rootDir = makeRootDir()
    const persistence = new FilePersistence({ rootDir, schedule: TEST_SCHEDULE })
    await seedAndFlush(persistence, { userId: 'alice' }, 'doc1', (doc) => {
      doc.getMap('ROOT').set('owner', 'alice')
    })
    await seedAndFlush(persistence, { userId: 'bob' }, 'doc1', (doc) => {
      doc.getMap('ROOT').set('owner', 'bob')
    })
  ...
  it('keeps plugin instances with different rootDir fully independent', async () => {
    ...
    const a = new FilePersistence({ rootDir: rootA, schedule: TEST_SCHEDULE })
    const b = new FilePersistence({ rootDir: rootB, schedule: TEST_SCHEDULE })
    await seedAndFlush(a, { userId: 'alice' }, 'doc1', (doc) => {
      doc.getMap('ROOT').set('secret', 'only-in-a')
    })
  ...
  it('replaces a committed snapshot via atomic rename: a read-only committed file does not block the next flush', async () => {
    ...
    const persistence = new FilePersistence({ rootDir, schedule: TEST_SCHEDULE })
    await seedAndFlush(persistence, { userId: 'alice' }, 'doc1', (doc) => {
      doc.getMap('ROOT').set('generation', 1)
    })
    ...
    await seedAndFlush(persistence, { userId: 'alice' }, 'doc1', (doc) => {
      doc.getMap('ROOT').set('generation', 2)
    })
  ...
  it('dispose cancels pending flush timers, leaves nothing written, and rejects further use', async () => {
    const rootDir = makeRootDir()
    const timer = createFakeTimer()
    const persistence = new FilePersistence({ rootDir, timer })
    const handle = await createFileHandleForTest(persistence, { userId: 'alice' }, 'doc1')
    handle.doc.getMap('META').set('docId', 'doc1')
    await persistence.saveDoc(handle)
