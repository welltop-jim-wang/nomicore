 ...
 *   ——extract 的方向反转孪生（issue #74）：先 validateLogicalSnapshot（逻辑失败保留
 *   完整 issues 引用透传），再构造 detached Yjs 子树，确认 ROOT 空置后单次 Y.transact
 *   安装；验证/构造失败 doc 零写入；不覆盖、不合并、不 fallback；事务内 observer
 *   抛错 loud 原样传播（不虚假声称回滚）；同步、错误经返回值传递。
 */
