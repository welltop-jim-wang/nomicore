 ...
 * 顶层崩溃边界收编（extract 侧 DOCRT-E100 / materialize 侧 prepare → DOCRT-E204、
 * ⑥ scratch 侧 → DOCRT-E201 变体 D；issue #87 起以 DerivedInvariantError 派生错误
 * 形式抛出，由捕获点按管线位置分类，对齐 evaluate.ts 手造 IR loud 边界）。移动使两侧共享同一实现，杜绝复制漂移。
 */
...
export function makeRefResolver(derived: DerivedSchema): (node: StructureNode) => StructureNode {
  ...
  return function resolve(node: StructureNode): StructureNode {
    ...
    while (cur.kind === 'ref') {
      // 环守卫先于 memo 命中判定：合法输入两序等价，手造环必须在此 loud 抛出（→ E100 / E204），
      // 而非经 memo 命中陷入无限循环（D8「递归无环守卫」意图；镜像 vfsl walkRefChain 语义）
      if (inFlight.has(cur.name)) throw new DerivedInvariantError(`结构 ref 环（${cur.name}）`);
      ...
      if (next === undefined) throw new DerivedInvariantError(`结构 ref 缺名（${cur.name}）`);
      memo.set(cur, next);
      cur = next;
    }
    return cur;
