 ...
 *      + META.schema.updatedAt 同事务安装（issue #282 / ADR-0017——schema 生命周期
 *      元数据随 generation 原子提交；既有嵌套 Y.Map 原实例复用，缺席/异型修复性安装）
 ...
 *      + ⑤-M：verifyMetaSchemaUpdatedAt（嵌套 Y.Map 载体 + updatedAt 严格同一）
...
export interface SchemaReplaceInput {
  ...
  /**
   * 【issue #282 / ADR-0017】当前 schema generation 的安装时间（UTC ISO 8601 字符串，
   * 由 namespace-runtime SCHEMA 写槽在槽内单点捕获时钟产生）。本 seam 在**同一事务**
   * 内将其写入 `META.schema.updatedAt`（嵌套 Y.Map——缺席/异型原值由本 seam 修复性
   * 安装，既有 Y.Map 载体身份保持复用），使 SCHEMA 与其生命周期元数据原子提交、
   * 复制时随 generation 同行。每次提交成功都推进——含语义等价/仅格式差异的替换
   * （时间戳描述「提交的 SCHEMA generation」，不据语义指纹相等推断）。
   */
  readonly schemaUpdatedAt: string;
}

/** ① 准备结局：ready（单事务所需载荷齐备）/ fail（零写入失败面）。
 *  【issue #282】ready 两分支均携带 metaMap（META 载体探针产物——SCHEMA 生命周期
 *  元数据与 SCHEMA 同事务提交，keep-root/replace-root 无差异）。 */
type PreparedSchemaReplace =
  | { kind: 'ready'; branch: 'keep-root'; schemaMap: Y.Map<unknown>; metaMap: Y.Map<unknown> }
  | {
      kind: 'ready';
      branch: 'replace-root';
...
export function replaceSchemaAndRoot(doc: Y.Doc, input: SchemaReplaceInput): ReplaceResult {
  ...
  transactGuarded(doc, () => {
    ...
    // 【issue #282 / ADR-0017】META.schema.updatedAt 同事务提交（schema 生命周期元数据
    //    随 generation 原子安装）：既有嵌套 Y.Map 原实例复用（载体身份保持）；缺席或
    //    异型原值（损坏/legacy）→ 修复性安装全新 Y.Map——SCHEMA 写路径即其生命周期
    //    元数据的唯一合法写者，修复权归本事务。
    const existingSchemaMeta = ready.metaMap.get('schema');
    const schemaMeta: Y.Map<unknown> = existingSchemaMeta instanceof Y.Map ? existingSchemaMeta : new Y.Map<unknown>();
    if (schemaMeta !== existingSchemaMeta) ready.metaMap.set('schema', schemaMeta);
    schemaMeta.set('updatedAt', input.schemaUpdatedAt);
  ...
  verifyMetaSchemaUpdatedAt(ready.metaMap, input.schemaUpdatedAt); // ③ ⑤-M（#282：生命周期元数据写后核验）
  if (ready.branch === 'replace-root') {
...
function prepareSchemaReplace(input: SchemaReplaceInput, doc: Y.Doc): PreparedSchemaReplace {
  try {
    ...
    // ①b2【issue #282】schemaUpdatedAt 形状守卫（纵深防御：服务 doc-runtime 直接
    //  调用方——杜绝 undefined/空串时间戳走私进 META.schema；runtime 路径由槽内
    //  时钟读数校验先行，结构性不可达）。违者 → 恰 1 issue path=[]，零写入。
    if (typeof input.schemaUpdatedAt !== 'string' || input.schemaUpdatedAt.length === 0) {
      return { kind: 'fail', issues: [{
        message: 'SCHEMA 生命周期元数据形状违规：schemaUpdatedAt 必须为非空 string（UTC ISO 8601）——无法执行原子 SCHEMA 替换',
        path: [],
      }] };
    }
    ...
    // ①c2【issue #282】META 载体探针（同款四级级联；异型 → ok:false 零写入——
    //  生命周期元数据绝不写入异型 META；缺席 → getMap 惰性创建，零 update）
    const metaProbe = probeMetaMap(doc);
    if (metaProbe.carrier !== 'Y.Map') {
      return { kind: 'fail', issues: [{
        message: `META 载体不是 Y.Map（期望 Y.Map，实际 ${metaProbe.carrier}）——无法执行原子 SCHEMA 替换（schema 生命周期元数据无处安装）`,
        path: [],
      }] };
    }
    const metaMap = metaProbe.map;
    // ①d 分支 prepare
    if (input.root.kind === 'keep-root') {
      ...
      return { kind: 'ready', branch: 'keep-root', schemaMap, metaMap };
    }
...
/** ①c2【issue #282】META 载体探针（镜像 probeSchemaMap 四级级联，只触碰 'META'
 *  名字空间）：仅 Y.Map 可继续（缺席 → getMap 惰性创建，零 update）；异型仅报 issue
 *  用；四级全失败 = 公共 API 造不出的第五种 META → 抛错归 ① 崩溃边界 DOCRT-E206。 */
function probeMetaMap(doc: Y.Doc): SchemaMapProbe {
  try {
    return { carrier: 'Y.Map', map: doc.getMap('META') };
  } catch { /* META 存在且非 Y.Map */ }
  try {
    doc.getArray('META');
    return { carrier: 'Y.Array' };
  } catch { /* 继续 */ }
  try {
    doc.getXmlFragment('META');
    return { carrier: 'Y.XmlFragment' };
  } catch { /* 继续 */ }
  try {
    doc.getText('META');
  ...
  throw new Error('META 载体探针全失败（公共 API 造不出第五种 META）'); // → ① 崩溃边界 E206
}

/**
 * ③ ⑤-M【issue #282 / ADR-0017】：META.schema 生命周期元数据写后核验——事务 cleanup
 * 派发窗口内若 observer 破坏本 seam 写入的载体/时间戳，此处把「嵌套 Y.Map 且
 * updatedAt 与安装值严格同一」承诺机制化（镜像 verifySchemaFourKeys 的双断言哲学；
 * 不断言嵌套 map size——既有载体的其余键不属本 seam 管辖，未来扩展键不受拒）。
 * 只读；任何偏离 → throw DOCRT-E201（写入已提交，不回滚、不补偿——W1 唯一相容形态）。
 */
function verifyMetaSchemaUpdatedAt(metaMap: Y.Map<unknown>, schemaUpdatedAt: string): void {
  const schemaMeta = metaMap.get('schema');
  if (!(schemaMeta instanceof Y.Map)) {
    throw new DocRuntimeFatalError(
      'post-commit-verification',
      true,
      'DOCRT-E201: META.schema 生命周期元数据安装完整性偏离：载体在事务提交后非 Y.Map——' +
        '疑似 observer 同步重入修改 META；写入已提交，不回滚、不补偿，doc 保持 observer 留下的实际状态',
    );
  }
  if (schemaMeta.get('updatedAt') !== schemaUpdatedAt) {
    throw new DocRuntimeFatalError(
      'post-commit-verification',
      true,
      'DOCRT-E201: META.schema 生命周期元数据安装完整性偏离：updatedAt 在事务提交后与安装值不同一——' +
