...
- `getSchema()`, `getMetadata()`, and `getActiveSchema()` return detached projections and never expose live writable Yjs references. `getActiveSchema()` carries the six-field active generation identity including `updatedAt` (UTC ISO 8601 install time of the active schema generation, or `null` for legacy namespaces without `META.schema` — ADR 0017); `getMetadata()` projects the nested `META.schema` Y.Map as a plain object.
...
- `replaceSchema(input)` compiles a proposed schema and atomically replaces SCHEMA plus, when supplied, the complete ROOT generation. A supplied ROOT is validated as-is; unknown top-level or nested keys fail loudly with zero writes. The same transaction commits `META.schema.updatedAt` (schema lifecycle metadata, ADR 0017): every committed replacement advances it — including semantically identical, formatting-only replacements — while rejected/zero-write outcomes leave it unchanged.
...
- **受保护字段**：hub 侧 SCHEMA+META 全键 / peer 侧 META 全键（除白名单 `{'schema'}`——ADR 0017：schema 生命周期元数据随 SCHEMA generation 同行，peer 收敛 hub 起源 `updatedAt`；冻结常量）；判据 = 内容投影相等（结构值规范化深比较——键序无关/数组有序/SameValue/白名单容器 toJSON 投影/契约外容器保守拒）；畸形字节 scratch 预演拒绝。
