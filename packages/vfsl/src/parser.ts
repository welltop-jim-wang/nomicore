 ...
 *
 * 资源界（#5 §15.2 → #7 R2 §4.6 → #6 §4.6 统一）：使 parseTypeExpr 递归的入口有
 * 三个——parseObjectType（`{`）/ parseMarkerType（五标记 `<`）/ parseRecordType
 * （`Record<`）——共用统一类型嵌套深度预算 `MAX_TYPE_NESTING = 100`（#7 曾更名
 * MAX_TYPE_DEPTH，#6 统一现名，值与 v1 承诺不变；当前嵌套深度：入口 +1、正常出口
 * -1，§10.7 权威读法）守卫递归栈与序列化两个资源界；`[]` 后缀串的第 k 个 `[`
 * 检查 `depth + k > MAX`（AST/IR 深度维度）；超限 → E100 资源上限口径，锚预算
 * 耗尽处容器构造起始记号（`{` / 标记名 / `Record` / 该 `[`）。
...
/** 类型嵌套深度预算（§4.6；统一计数器：对象 `{`、marker `<`、`Record<` 三个递归
 * 入口共用 + `[]` 后缀串按 depth+k 计；不导出、不进公共面，变更须回总控走设计修订）。 */
...
class Parser {
  ...
  private parseIdentType(tok: Token): AstType {
    ...
    if (MARKER_NAMES.has(v)) {
      if (this.peekPunct('<')) {
        return this.parseMarkerType(tok); // ★ 完整解析（docs 挂载随 #7 JSDoc M3 回收）
      }
      throw this.err(ErrCode.E100, `裸引用保留名: ${v}（判定顺序第 7 条）`, tok);
    }
    if (v === 'type' || v === 'Pattern') {
  ...
  private parseMarkerType(tok: Token): AstType {
    ...
    try {
      ...
      return { kind: 'marker', marker: tok.value as MarkerName, arg, pos: posOf(tok), docs };
    } finally {
      this.depth -= 1;
    }
  }
