...
describe('§9.4 message 4 KiB 字节预算（R2/E-c2 JSON 字面量基准）', () => {
  it('恰 4096B 不截断；4097B 截断且序列化字节 ≤ 4096', () => {
    const exact = project({ items: [{ message: 'a'.repeat(4096), path: [] }] })
    expect(exact.items[0]!.message).toBe('a'.repeat(4096))
    expect('truncated' in exact).toBe(false)

    const over = project({ items: [{ message: 'a'.repeat(4097), path: [] }] })
  ...
  it('多字节字符骑界：3B 字符不拆分（截在字符前）', () => {
    ...
    const proj = project({ items: [{ message: 'a'.repeat(4082) + '€'.repeat(3) + 'b'.repeat(10), path: [] }] })
  ...
  it('astral 字符（代理对）不拆分：输出无 lone surrogate', () => {
    const proj = project({ items: [{ message: 'a'.repeat(4083) + '😀'.repeat(5) + 'b'.repeat(10), path: [] }] })
  ...
  it('1365 个 lone surrogate（字面量 8190B）> 4096 → 截断且截断后序列化字节 ≤ 4096', () => {
    const proj = project({ items: [{ message: '\ud800'.repeat(1365), path: [] }] })
...
describe('§9.4 path / 条目数预算', () => {
  it('path 257 段 → 保前 256 段 + truncated', () => {
    ...
    const proj = project({ items: [{ message: 'm', path }] })
  ...
  it('string 段 1025B → 截断到 ≤1024B（前缀 1010B + marker 14B）', () => {
    const proj = project({ items: [{ message: 'm', path: ['a'.repeat(1025)] }] })
  ...
  it('number 段原样保留（含 0）；string/number 混合 path 保序', () => {
    const proj = project({ items: [{ message: 'm', path: ['a', 1, 'b', 2] }] })
  ...
  it('1001 条 → 保前 1000 条 + truncated + originalCount=1001；恰 1000 条 → 无截断标记', () => {
    ...
    const proj2 = project({ items: exact })
...
describe('§9.4 策略投影（policy 自标，ADR 0011 §数据保护）', () => {
  it('redacted：message→«redacted»，code/path 保留，policy 自标', () => {
    const proj = project(
      { items: [{ code: 'registry.e1', message: 'secret failure detail', path: ['a', 1] }] },
      'redacted',
    )
  ...
  it('none：空 items（策略即承诺不捕获）', () => {
    const proj = project({ items: [{ message: 'm', path: [] }] }, 'none')
    expect(proj).toEqual({ policy: 'none', items: [] })
  })

  it('code 截断至 256B（R2 预算基准下 242B 前缀 + marker 14B）', () => {
    const proj = project({ items: [{ code: 'c'.repeat(300), message: 'm', path: [] }] })
...
describe('§9.4 畸形条目丢弃（§4.2/§6.2）', () => {
  it('缺 message / path 非数组 → 整条丢弃，originalCount 只计有效', () => {
    const proj = project({
      items: [
        { path: [] } as never,
    ...
    // R4/C-3：畸形丢弃与截断同现同缺（§6.2）——丢弃过条目 ⇒ truncated && originalCount 同现
    expect(proj.truncated).toBe(true)
  ...
  it('undefined 段（稀疏数组 hole 读出）→ 整条丢弃', () => {
    ...
    holey.length = 2
    holey[1] = 'x'
    const proj = project({ items: [{ message: 'hole', path: holey }, { message: 'ok', path: [] }] })
    expect(proj.items).toHaveLength(1)
  ...
  it('-0 number 段 → 投影归一为 +0（内存对象即 0，非 -0）', () => {
    const proj = project({ items: [{ message: 'm', path: [-0, 1] }] })
