...
- **Plugin configuration**：`createNamespaceRegistryPlugin({ idleTimeoutMs? })`；实例静态角色 exclusively 来自注入的 `@nomicore/instance` service，插件不再接受独立 `role`。
...
- `ctx.nomicoreInstance` from `@nomicore/instance` (immutable `{ instanceId, role }`);
