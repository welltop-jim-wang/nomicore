...
- **ADR 0019**（已接受，issue #295 设计冻结）：本设计是其显式履行票；wire 冻结值以协议文档为唯一权威（ADR 0019 L4），配置语义理据权威保留于 ADR 0019、机制键语义显式转引协议 §17（L61）。
- **ADR 0013**（已接受）：其非目标 #4 的划除与接替登记（L117）、ADR 0019 L116 对「未协商端逐字节不变」纪律的限定范围偏离登记均显式在册——六字段旧形态不构成约束；`CAP_CHUNKED_UPDATE` 协商门是 ADR 0013 已实现内容、ADR 0019 非目标 #5 明确不修订。
...
- **ADR 0019 事实前提复核**：PR #241 OPEN 未合并（本次 `gh pr view 241` 实测）；分支 codec 现状六字段（`payloads.ts` `decodeUpdateChunk` 实测：`namespaceId` 起始、无 kind/绑定块）；`messages.ts` `UpdateChunkMsg` 以 `kind:'UPDATE_CHUNK'` 为判别键、无 `transferKind` 成员——「旧形态从未发布、无兼容负担」前提成立，设计 D1–D7 的改写面有 ADR 0019 L36–37/L100 直接授权。
...
| `ReplicationLimits` + 两必填 readonly number 键 + `DEFAULT_REPLICATION_LIMITS` 两键 4 MiB + `validateLimits` 无条件 `positiveSafeInteger` 值门 + `LIMIT_KEYS` +2 | ADR 0019 配置表 L58–59（缺省 4 MiB、约束链②）；§17 L578–579/L590–609（缺省值 + 启动响亮验证块；值门无条件先例 = #244 键 `maxChunksPerUpdate ≥ 1`，`validate.ts` L152/L154 实测同构）；缺省 4 MiB 恒合法 → 零非追溯性面 | 无冲突 |
...
| 破坏性演化作用域（必填成员使全部构造点编译期表态） | 包未发布、workspace `*` 内部消费、PR #241 未合入（实测）、同版本部署假设（ADR 0019 L14–18）——无仓外/跨版本消费方；§10 矩阵穷举仓内构造点 | 无冲突（R40 登记实现期核对义务） |

### 2.2 0x42 wire 字段序（设计 D3/D4/D7）——无冲突

- **读序/写序**：`kind varUint` 首位 → `namespaceId` → `transferId`（≥1、0 非法）→ `chunkIndex` → `chunkCount`（≥1、idx<count）→ `totalBytes` → [绑定块] → `bytes`——与 §5 L116、§10.3 L313 字段表、L323 单形态段、ADR 0019 L24–34 代码块**逐字同源**；绑定块位置（`totalBytes` 之后、`bytes` 之前）逐字一致。encode 写序 = 读序镜像满足 L321「encode/decode 同一套」。
- **kind 非法值即拒**：`kind ∉ {0,1,2}` → `MALFORMED_FRAME` 于首字段读取处——对齐 ADR 0019 L37「恶意声明在第一个字节流入前即可拒绝」；旧六字段首字节 0x23=35 与合法 kind 集合不相交 → 旧形态自动作废（SA6 probe A 实测佐证）。
- **golden 改写**：3 条 golden 改 kind {0,1,2}、计数 21 不变、绑定块形态由契约冻结向量 `KIND1_FIRST`/`KIND2_FIRST` 逐字节锁定——ADR 0019 L100「#242 的 0x42 golden vectors 在本分支内改写为单形态」直接授权；§22 L701「kind 首字段 + 首 chunk 绑定块的 golden vectors…由实现 ticket 交付」由此兑现（交付物 = 改写后 golden + 契约冻结向量，收口措辞精度见 R39）。
...
- **存在性/位置 iff**：`当且仅当 kind≠0 ∧ chunkIndex=0`（kind=1 → `replicationId varString + replicationEpoch varUint`；kind=2 → `syncRoundId varUint`）——§10.3 L323、§5 L116、ADR 0019 L32/L37–43 逐字同源；违例向量收敛 `MALFORMED_FRAME`（R8/R9/R10 契约向量与 D3 收敛路径推导一致）。
...
- **transferId 语义锁定**：uint32、≥1、0 非法、三 kind 同一字段位/同一计数器（§5 L116、§10.3 L315/L325、ADR 0013 L32 + ADR 0019 L28、CONTEXT.md「分块复制传输」L154）；计数器本体留发送端（复用 `update-channel.ts` 既有 `nextTransferId`、不新增第二计数器）——CONTEXT.md「UPDATE_CHUNK」codec 无状态纪律（R33）一致。
...
- **R32 · 【轻微冲突·接续】issue 标题文本仍与 ADR 0019 冲突**：标题「…CAP_CHUNKED_SYNC 协商、0x42 kind 双形态 codec…」仍逐字命中 ADR 0019 明确拒绝方案 #1（L87）与 CONTEXT.md「同版本部署假设」Avoid 条。设计已按正文/AC 单形态落地（§1 非目标显式废弃标题措辞）——不阻塞；建议 reporter 改题（如「0x42 kind 首字段单形态 codec、聚合上限配置链」）并在派发词中声明标题废弃。仓内规范文档零「双形态/CAP_CHUNKED_SYNC」命中（维持）。
