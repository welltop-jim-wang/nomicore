...
3. In one Cordis `Context`, install in dependency order:
   ...
   5. `createNamespaceRegistryPlugin()`, which reads role from the Instance service and has no role configuration; to enable the optional namespace diagnostic change log, pass a host-owned adapter (e.g. `createFileDiagnosticLog` from `@nomicore/namespace-diagnostic-log`) as the second `host: { diagnosticLog }` argument;
