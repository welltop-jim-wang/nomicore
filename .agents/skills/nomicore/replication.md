...
For `resync-required{cause:"send-failed"}`, read the appended `reason` before choosing a fix: `update-too-large` (with `updateBytes`/`maxUpdateBytes`) means shrink the business update or raise the configured limit; `send-frame-rejected` means inspect the connection/backpressure state machine using the attached `channelState`/`connectionState`/queue/in-flight counts and optional `bufferedAmount`. Other `cause` values carry no `reason` field. An oversized update dropped while the send queue is non-empty (F4 liveness, no resync declared) reports `update-dropped{reason:"update-too-large"}` with the same safe numeric context instead—counting invariant: exactly one observer event per oversized drop, and repair comes from the next reconciliation round.
