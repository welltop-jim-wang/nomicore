...
5. Start Hub before Peer. Hub plugin readiness means its listener is active with authentication and authorization wired; Peer plugin readiness means its controller/dial loop started and does not imply Hub connectivity. Configure/add the Peer target, then use `waitForLive(namespaceId)` before starting Peer work that requires a live replica. Do not equate an UPDATE ACK with disk flush or quorum durability.
